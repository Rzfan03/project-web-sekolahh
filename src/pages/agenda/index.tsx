          ...
          <div className="lg:col-span-2">
            {loading ? (
              <div className="space-y-4">
                {[0, 1, 2].map((i) => (
                  <div key={i} className="flex gap-5 rounded-2xl border border-stone-200 bg-white p-6">
                    <div className="h-20 w-16 flex-none animate-pulse rounded-xl bg-stone-100" />
              ...
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-stone-200 py-24 text-center">
                ...
                <p className="mt-4 text-sm text-stone-500">Belum ada agenda.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {data.map((a) => {
                  ...
                  return (
                    <article
                      key={a.id}
                      className="flex items-start gap-5 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm transition-shadow hover:shadow-md"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <h2 className="font-display text-lg font-extrabold leading-snug text-stone-900">{a.judul}</h2>
                        ...
                        {a.keterangan && (
                          <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-stone-600">{a.keterangan}</p>
                        )}
                      </div>
                    </article>
                  )
          ...
          <aside className="space-y-6">
            <section className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-display text-lg font-extrabold text-stone-900">Rilis Berita</h2>
              </div>
              <div className="mt-5 divide-y divide-stone-100">
                {berita.map((b) => (
                  <Link key={b.id} to={`/berita/${b.slug}`} className="group block py-3 first:pt-0 last:pb-0">
                    <p className="line-clamp-2 text-sm font-semibold leading-snug text-stone-700 transition-colors group-hover:text-orange-600">
                      {b.judul}
                    </p>
                    <p className="mt-1 text-xs text-stone-400">{formatShort(b.created_at)}</p>
                  </Link>
                ))}
                {berita.length === 0 && (
                  <p className="py-4 text-sm text-stone-400">Belum ada berita.</p>
                )}
              </div>
              <Link
                to="/berita"

...
const upload = (folder, field) => multer({
  ...
  fileFilter: (_, file, cb) => cb(null, ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'].includes(file.mimetype)),
...
const errHandler = (err, req, res, next) => { console.error(err); res.status(500).json({ message: 'Internal server error' }) }
...
app.get('/api/berita', async (req, res) => {
  ...
  const q = supabase.from('articles').select('*, admins(username)', { count: 'exact' }).eq('status', 'published').order('created_at', { ascending: false }).range(p.from, p.to)
